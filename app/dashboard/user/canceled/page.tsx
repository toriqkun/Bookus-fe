...
export default function CanceledPage() {
  ...
  const fetchCanceled = async () => {
    try {
      const { data } = await api.get("/bookings", { params: { status: "CANCELED" } });
  ...
  // if (!bookings.length) return <p className="p-6 text-center text-gray-500"></p>;
  ...
  return (
    <div className="p-6">
      ...
      {bookings.length === 0 ? (
        <p className="text-gray-500 text-center">No canceled bookings found.</p>
      ) : (
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-6">
          {bookings.map((b) => (
            <div key={b.id} className="bg-white rounded-md shadow-md hover:shadow-xl overflow-hidden">
              ...
              <div className="p-4 text-center">
                ...
                <p className="mt-3 bg-red-200 text-red-700 text-sm font-semibold py-1 px-5 rounded-full inline-block">{b.status}</p>
              </div>
            </div>
          ))}
        </div>
