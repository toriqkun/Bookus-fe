...
interface Booking {
  ...
  service: { title: string; coverImage?: string };
  user: { name: string };
}

export default function BorrowedAdminPage() {
  ...
  const fetchBorrowed = async () => {
    try {
      const { data } = await api.get("/bookings/all", { params: { status: "BORROWED" } });
  ...
  const handleComplete = async (id: string) => {
    const t = toast.loading("Processing returns...");
    try {
      await api.patch(`/bookings/status/${id}`, { status: "COMPLETED" });
      toast.success("Book successfully returned!", { id: t });
      await fetchBorrowed();
    } catch {
      toast.error("Book failed to be returned!", { id: t });
    }
  };
  ...
  return (
    <div className="p-6">
      <div className="flex items-center gap-2 mb-6">
        ...
        <h2 className="text-2xl font-semibold">Being borrowed 📚</h2>
      </div>

      {loading ? (
        <p className="text-gray-500 text-center">Load data...</p>
      ) : bookings.length === 0 ? (
        <p className="text-gray-500 text-center">There are no books currently on loan.</p>
      ) : (
        <div className="grid sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-6">
          {bookings.map((b) => {
            const timer = timers[b.id] || 0;
            const canComplete = timer <= 0;
            ...
            return (
              <div key={b.id} className="bg-white rounded-md shadow-sm hover:shadow-md transition overflow-hidden">
                ...
                <div className="p-4 text-center">
                  ...
                  {/* Countdown */}
                  {!canComplete && <p className="text-xs text-red-500 mt-1">Remaining time: {formatTimer(timer)}</p>}

                  <button
                    onClick={() => handleComplete(b.id)}
                    disabled={!canComplete}
                    className={`mt-3 px-4 py-2 text-sm rounded-md transition ${canComplete ? "bg-purple-600 hover:bg-purple-700 text-white" : "bg-gray-300 text-gray-500 cursor-not-allowed"}`}
                  >
                    {canComplete ? "Completed" : "Completed"}
                  </button>
                </div>
              </div>
            );
